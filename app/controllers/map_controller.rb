module Controllers
  class MapController
    ...
    TILE_WIDTH = 32
    TILE_HEIGHT = 32
    MOVE_ZONE_BUFFER_X = 8 * TILE_WIDTH
    MOVE_ZONE_BUFFER_Y = 6 * TILE_HEIGHT
    ...
    def self.tick(args)
      ...
      if player_x_offset < MOVE_ZONE_BUFFER_X
        map.x = [min_x, map.x - TILE_WIDTH].max
      elsif player_x_offset > (1280 - MOVE_ZONE_BUFFER_X)
        map.x = [map.x + TILE_WIDTH, max_x].min
      end
      if player_y_offset < MOVE_ZONE_BUFFER_Y
        map.y = [min_y, map.y - TILE_HEIGHT].max
      elsif player_y_offset > (720 - MOVE_ZONE_BUFFER_Y)
